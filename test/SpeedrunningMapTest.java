...
public class SpeedrunningMapTest {
    ...
    public void singleKeyTest(){
        ...
        GraphMap g = new GraphMap(PoI, distance);
        Map<String,Double> unlock = new HashMap<>();
        unlock.put("key:start->finish",3.0);
        SpeedrunningMap e = new SpeedrunningMap(g, "start", "finish",unlock);
...
//        assertEquals(shouldBe, e.getGraph().toStringAll());
        ...
        assertEquals("[start, key, start, finish]",e.getSuperSimpleList().toString());
    }
    ...
    public void testDoubleIndepentKey() {
        ...
        GraphMap g = new GraphMap(poi,movement);
        Map<String, Double> map = new HashMap<>();
        map.put("3:1->2", 10.0);
        map.put("1:3->4",1.0);
        SpeedrunningMap e = new SpeedrunningMap(g, "0", "5", map);
        assertEquals("[0, 1, 1, 0, 3, 4, 5]", e.getSimpleList().toString());
        assertEquals("[0, 1, 0, 3, 4, 5]", e.getSuperSimpleList().toString());
    }
    ...
    public void testDoubleDepedepentKeyWords() {
        ...
        SpeedrunningMap e = new SpeedrunningMap(g, "start", "finish", map);
        assertEquals("[start, key1, key1, start, key2, key2, start, key1, finish]", e.getSimpleList().toString());
        assertEquals("[start, key1, start, key2, start, key1, finish]", e.getSuperSimpleList().toString());
    }
    ...
    public void testTripleDepedepentKey() {
        ...
        assertEquals("[0, 3, 4, 3, 0, 1, 2, 1, 0, 3, 4, 5]", e.getSuperSimpleList().toString());
    ...
    public void nonUniqueSingleKeyKeyTest(){
        ...
        SpeedrunningMap s = new SpeedrunningMap(g, "start", "end", map);
        assertEquals("[start, key1~1, key1~1, key2~1, key2~1, gate1~1, gate2~1, end]",s.getSimpleList().toString());
        assertEquals("[start, key1~1, key2~1, gate1~1, gate2~1, end]",s.getSuperSimpleList().toString());
    }
...
//        assertEquals("[start, key3~2, key3~2, start, key1~1, key1~1, gate2~2, gate3~1, end]",s.getSimpleList().toString());
        assertEquals("[start, key3~2, start, key1~1, gate2~2, gate3~1, end]",s.getSuperSimpleList().toString());
        assertEquals(6.0,s.getShortestDistance(), .1);
    }
